...
def handle(old_table, new_class)
  puts "*** #{old_table}"
  in_name = IN_BASE + "/#{old_table}_export.csv"
...
handle("DocuOverTrefwoorden", Ephemerum)
handle("IcoOverTrefwoorden", IcoTitle)
handle("TrefwoordenBijTijdschrift", Periodical)
handle("VerwervingOverTrefwoorden", Donation)
