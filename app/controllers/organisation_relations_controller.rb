class OrganisationRelationsController < DetailsController
  include Userstamp
  def destroy
    if admin?
  ...
  def update
    if admin?
      ...
      prev_direction = params[:organisation_relation][:direction]
      direction = params[:organisation_relation][:organisation_relation_type_id]

      if (prev_direction == "from" && direction =~ /-to$/) || (prev_direction == "to" && direction =~ /-from$/)
        @from = @relation.organisation_to
        @to = @relation.organisation_from
        @relation.organisation_from = @from
        @relation.organisation_to = @to
      end
